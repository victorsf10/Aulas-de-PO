...
while opcao != 5:
    ...
    opcao = int(input("-> "))
    if opcao == 1:
       #Caso queria inserior os elementos manualmente com valores altos.
        #lista = []
        #tam = int(input('Digite quantos valores seram incluidos: '))
        #for x in range(0, tam):
        #   x = random.randint(1, tam)
        #    lista.append(x)
      # Caso queira inserir os elementos aleatoriamente
      # sem precisar digitar valor.
        lista_entrada = [100, 500, 1000, 10000]
        for x in lista_entrada:
          lista = list(range(0, x + 1))
          random.shuffle(lista)
        inicioIns = timeit.default_timer()
        for x in lista:
            arv.inserir(x)
        fimIns = timeit.default_timer()
        resultadoIns = fimIns - inicioIns
        print(lista)
        print("O tempo para inserir os elementos é de: %.10f " %resultadoIns)

    elif opcao == 2:
        x = int(input(" Informe o valor -> "))
        inicioRem = timeit.default_timer()
        if arv.remover(x) == False:
            print(" Valor nao encontrado!")
        fimRem = timeit.default_timer()
        resultadoRem = fimRem - inicioRem
        print("O tempo para remover um determinado elemento é de: %.10f " %resultadoRem)
    elif opcao == 3:
        x = int(input(" Informe o valor -> "))
        ...
        print("O tempo para encontrar foi: %.10f" % resultado)


    elif opcao == 4:
        inicioPos = timeit.default_timer()
        arv.caminhar()
